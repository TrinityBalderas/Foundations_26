...
const myCallBack = (array, oldArray) => array.length <= 15 ? console.log( 
    ...
    const uniqUsingLoops = (array, callback = defaultCallBack, showDebug = false) => {
        ...
        for (let outer = 0; outer < noDupesArray.length; outer++) {
          for (let inner = outer + 1; inner < noDupesArray.length; inner++) {
            counter++; // debug
...
// uniqUsingLoops(arrayMaker(20000,10), defaultCallBack,true)
// uniqUsingSet(arrayMaker(20000,10), defaultCallBack,true)
uniq(arrayMaker(20000,10), defaultCallBack,true)